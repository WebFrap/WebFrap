...
//////////////////////////////////////////////////////////////////////////////*/
  ...
  public function pagingMenu ($linkTarget, $ajax = true)
  {
    ...
    for($nam = $startPos; $nam < $endPos; ++ $nam) {
      ...
      if ($ajax) {
        $urlClass = ($nam == $activPos) ? 'class="ui-state-active wcm wcm_req_page ' . $this->searchForm . '"' : 'class="wcm wcm_req_page ' . $this->searchForm . '"';
      } else {
        $urlClass = ($nam == $activPos) ? 'class="ui-state-active"' : '';
      }
      
      $title = $this->i18n->l('Show the next {@Y@} {@X@} Entires', 'wbf.label', array(
        'Y' => $nam, 'X' => $this->stepSize
      ));
  ...
  public function menuFootActions ()
  {
    ...
    $html = Wgt::icon('control/refresh.png', 'xsmall', array(
      'alt' => 'refresh', 'title' => 'refresh the table', 'onclick' => '$R.form(\'' . $this->searchForm . '\')'
    ));
    
    $html .= ' &nbsp;&nbsp;|&nbsp;&nbsp;';
    
    $html .= Wgt::icon('mimetype/excel.png', 'xsmall', array(
      'alt' => 'export excel', 'title' => 'export all datasets as excel', 'onclick' => "\$R.form('{$this->searchForm}','&amp;export=excel',{append:true,ajax:false} )"
    ));
    $html .= Wgt::icon('mimetype/xml.png', 'xsmall', array(
      'alt' => 'export xml', 'title' => 'export all datasets as xml', 'onclick' => "\$R.form('{$this->searchForm}','&amp;export=xml',{append:true,ajax:false} )"
    ));
    /*
    $html .= Wgt::icon
    (
    ...
    */
    ...
    $html .= Wgt::icon('control/cancel.png', 'xsmall', array(
      'alt' => 'cancel', 'title' => 'cancel all filters', 'onclick' => '$S(\'table#' . $this->id . '-table\').grid(\'cleanFilter\');'
    ));
