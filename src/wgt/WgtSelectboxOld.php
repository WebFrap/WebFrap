...
////////////////////////////////////////////////////////////////////////////////
  ...
  public function niceElement( $attributes = array() )
  {
    ...
    $this->attributes['class'] = isset($this->attributes['class'])
      ? $this->attributes['class'].' wgt-behind'
      : 'wgt-behind';
    ...
    {
      $this->attributes['onclick'] = "\$S('input#display-{$id}').val(this.options[this.selectedIndex].innerHTML);";
      $icon     = Wgt::icon('control/arrow_down.png', 'xsmall', 'down' );

      $appendButton = <<<HTML
<button class="wgt-button append wgt-overlay embed" onclick="\$S('#{$id}').click();return false;" >{$icon}</button>
HTML;
    ...
    $html = <<<HTML
      <div style="position:relative;" class="inline" >{$element}<input class="wgt-overlay embed medium wgt-ignore {$attrRo}" value="{$this->activValue}" type="text" name="display-{$this->attributes['name']}" id="display-{$id}" />{$appendButton}</div>
...
HTML;
    ...
    else
    {
      ...
      else
      {
        ...
        $buttonDropDown = <<<HTML
<button class="wgt-button append wgt-overlay" onclick="\$S('#{$id}').click();return false;" >{$icon}</button>
HTML;
    ...
    <div class="wgt-box input" id="wgt-box-{$id}" >
      ...
      <label class="wgt-label" for="{$id}" >{$this->texts->beforeLabel}{$this->label}{$this->texts->afterLabel} {$required}{$this->texts->endLabel}</label>
      {$this->texts->middleBox}
      <div class="wgt-input {$this->width}" style="position:relative;" >{$element}<input class="wgt-overlay medium wgt-ignore{$classRo}" value="{$this->activValue}" type="text" name="display-{$this->attributes['name']}" id="display-{$id}" />{$buttonDropDown}{$this->texts->afterInput}</div>
      {$this->texts->bottomBox}
      <div class="wgt-clear tiny" >&nbsp;</div>
    </div>
