 ...
 */
class MyMessage_Table_Query_Postgresql extends LibSqlQuery
{
/*//////////////////////////////////////////////////////////////////////////////
// attributes
//////////////////////////////////////////////////////////////////////////////*/
 ...
 /**
  ...
  public function setCols($criteria )
  {
    ...
    (
      ...
      'wbfsys_message_receiver.id_status as "wbfsys_message_receiver_id_status"',
          ...
          if ( strpos($condition['free'], ',' ) )
          {
            ...
            foreach($parts as $part )
            {
              ...
              if ( '@' == $safePart[0] )
              {
                ...
                $criteria->where
                ('(
                  UPPER(core_person_lastname) = UPPER(\''.$safePart.'\')
                    OR UPPER(core_person_firstname) = UPPER(\''.$safePart.'\')
                    OR UPPER(wbfsys_role_user_name) = UPPER(\''.$safePart.'\')
                )');
              }
              else 
              {
  ...
  public function appendFilter($criteria, $condition, $params  )
  {
    ...
    if (!isset($condition['filters']['mailbox'] ) )
    {
      ...
      if (!$condition['filters']['archive'] )
      {
        ...
        ( 
          " (
              wbfsys_message_receiver.id_receiver = " .$user->getId()."
                and 
              wbfsys_message_receiver.id_status IN( ".EMessageStatus::IS_NEW.", ".EMessageStatus::OPEN." )
            )
              or
            (
              wbfsys_message.id_sender = ".$user->getId()."
                and
