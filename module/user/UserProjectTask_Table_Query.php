 ...
 * @copyright Webfrap Developer Network <contact@webfrap.net>
 */
class UserProjectTask_Table_Query
  extends LibSqlQuery
{
 ...
 /**
  ...
  public function checkCharBegin( $criteria, $params )
  {
      ...
      if( $params->begin )
      {
        ...
        if( '?' == $params->begin  )
        {
          $criteria->where( "project_task.name ~* '^[^a-zA-Z]'" );
        }
        else
        {
          $criteria->where( "upper(substr(project_task.name,1,1)) = '".strtoupper($params->begin)."'" );
        }
...
}//end class UserProjectTask_Table_Query
